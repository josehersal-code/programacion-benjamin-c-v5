...
function PlanningPage({history=false}){
  ...
  async function duplicate(s){const {data:bl}=await supabase.from("session_blocks").select("*").eq("session_id",s.id);const {data:n,error}=await supabase.from("sessions").insert({team_id:TEAM_ID,mesocycle_id:s.mesocycle_id,session_date:s.session_date,kind:s.kind,goal:s.goal,goalkeeper_notes:s.goalkeeper_notes,notes:s.notes,status:"planned"}).select("*").single();if(error)return alert(error.message);if(bl?.length)await supabase.from("session_blocks").insert(bl.map(b=>({session_id:n.id,exercise_id:b.exercise_id,part:b.part,sort_order:b.sort_order})));await ensureTrainingActivity(n,settings.trainingStart);load()}
