...
function CalendarPage(){
  const [month,setMonth]=useState(new Date(2026,8,1)),[sessions,setSessions]=useState([]),[matches,setMatches]=useState([]),[conv,setConv]=useState(null),[editSession,setEditSession]=useState(null);
  async function load(){const {data:s}=await supabase.from("sessions").select("*").eq("team_id",TEAM_ID);setSessions(s||[]);await ensureSaturdayMatches();const {data:a}=await supabase.from("activities").select("*").eq("team_id",TEAM_ID).eq("type","match").order("activity_date");setMatches(a||[])}
  useEffect(()=>{load()},[]);
